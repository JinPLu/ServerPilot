"""Windows and static Web UI lock the accepted server-group presentation."""
...
WEB_PAGE = (ROOT / "src" / "serverpilot" / "web" / "templates" / "page.html").read_text(encoding="utf-8")
...
def test_web_dashboard_groups_existing_list_without_storage_group() -> None:
    assert "groupedEndpointSections" in WEB_JS
    assert "ungrouped last" in WEB_JS or "ungrouped: true, items: ungrouped" in WEB_JS
    assert "storage_group" not in WEB_JS
    assert "server-group-heading" in WEB_JS
    assert "id=\"groups-dialog\"" in WEB_DASHBOARD
    assert 'name="server_group_id"' in WEB_DASHBOARD
    assert "未分组（按单机申请）" in WEB_DASHBOARD
    assert "id=\"quick-claim-form\"" in WEB_DASHBOARD


def test_web_requests_page_claim_form_selects_groups() -> None:
    assert 'name="server_group_id"' in WEB_PAGE
    assert "未分组（按单机申请）" in WEB_PAGE
    assert "best-fit" in WEB_PAGE


def test_web_claim_posts_group_constraint_and_keeps_ungrouped_server_path() -> None:
    assert "constraints.server_group_ids = [groupID]" in WEB_JS
    assert 'groupID !== "__ungrouped__"' in WEB_JS
    assert 'fetch("/api/v1/claims"' in WEB_JS
    assert 'fetch("/api/v1/server-groups"' in WEB_JS or "/api/v1/server-groups/" in WEB_JS
    assert "environment_notes" in WEB_JS
    assert "os.environ" not in WEB_JS


def test_web_ssh_grouped_mutations_send_override_not_effective_path() -> None:
    builder = WEB_JS.split("const sshMutationBody", 1)[1].split("const requestSshPreview", 1)[0]
    assert "body.server_group_id = serverGroupId" in builder
    assert "body.workspace_path_override = workspacePathOverride ?? null" in builder
    assert "environment_notes" not in builder
    grouped = builder.split("if (serverGroupId)", 1)[1].split("} else if", 1)[0]
    assert "workspace_path_override" in grouped
    assert "body.workspace_path =" not in grouped
    ungrouped = builder.split("} else if (workspacePath)", 1)[1]
    assert "body.workspace_path = workspacePath" in ungrouped
    assert "workspace_path_override" not in ungrouped
    assert 'name="server_group_id"' in WEB_DASHBOARD
