...
- **An agent can now register a server into a group, and pick the right observation profile.** `gpu_add_server` / `gpu_update_server` did not accept `server_group_id`, so a server registered through them could never be selected; the default profile also named the most special case, which connects to an ordinary machine and reports no cards.
- **The state page no longer claims occupancy is running when it cannot see the card.** It used to replay the last recorded "running"; it now says that a card it cannot see cannot confirm what is on it.
