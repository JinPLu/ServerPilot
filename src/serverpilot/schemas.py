...
class EndpointCreate(StrictModel):
    ...
    # One default, shared with ``EndpointConfig``. There is only one built-in
    # profile now, so a caller that omits the field cannot pick the wrong one;
    # a value that is not ``linux`` names a locally discovered plugin.
