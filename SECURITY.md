...
- The SSH user and the remote `serverpilot-collect` entrypoint are trusted. Admission is fail-closed on *reported* facts: stale telemetry, connection errors, unknown processes, and resource conflicts refuse allocation. A replaced or malicious collector that omits compute processes yields `AVAILABLE`.
