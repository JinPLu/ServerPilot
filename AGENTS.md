...
- `project_id + task_ref` 是工作任务的稳定身份。默认 Agent 路径严格只有 `gpu_status(server_id?, lease_id?)`、`gpu_apply(server_group_id?, server_id?, gpu_count=1, task?)`、`gpu_release(lease_id)` 三个工具；先评估组级路径/环境/数据权重说明与容量并选 `server_group_id`，由调度器在组内 best-fit；`server_id` 仅用于 scheduler/plugin 或未分组兼容，不得用来钉住已分组的 direct 主机；遥测只随租约投影，可申请容量按服务器与型号汇总；连接与远端工作目录逐服务器投影，不逐卡重复；不依赖特定客户端的身份、UI 标题或专用环境变量。`task` 使用用户任务名或当前目标的简短人类可读概括。ServerPilot GUI 不实现消息、收件箱、未读状态或通信表单。
