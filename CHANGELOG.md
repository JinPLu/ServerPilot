...
- CPU 核数、内存总量、最高温度、显存合计（绝对 GB）和远端工作区完整路径移入详情弹窗新增的`主机`卡片。工作目录此前在行里只能截成 `…Data/tmp/ljp`，截断后不携带任何信息；这些事实也仍在行的 tooltip 里。
...
- 使用情况页与服务器详情弹窗此前把整页折叠成一个元素，屏幕阅读器只能读到一句概述，读不到里面的任何按钮和数值；现在逐项可读。
...
- 字阶加入 26pt 展示档，最大最小之比从 1.70 提到 2.60，界面上终于有元素能当主角；所有数字改为等宽数字，刷新时不再抖动。
