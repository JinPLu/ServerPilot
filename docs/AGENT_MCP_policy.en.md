...
`no_capacity` and `group_selection_required` mean no allocation or queue; `busy_gpus` names holders. Refresh once, then wait. Retry `Transport closed` once. Leases persist until released, App correction, or idle reclaim; a claim you keep asking about with `gpu_status(lease_id=…)` is not reclaimed as abandoned. Confirm each `released`, and claim only GPUs you will use because an idle one is returned on its own.
