...
默认的 `serverpilot-mcp` 只暴露 `gpu_status`、`gpu_apply`、`gpu_release` 三个工具。scheduler、通用资源、endpoint 管理和低层租约兼容工具仍然保留，但只有设置 `SERVERPILOT_MCP_PROFILE=advanced` 才会出现在工具发现结果里。默认配置不需要 `enabled_tools` 白名单。
...
1. `gpu_status(server_id?, lease_id?)` 分三组回答三个问题，遥测只出现在占用归属于调用者的那一组。**连接与工作目录逐服务器返回一次**：`servers[]` 给出 `server_id`、结构化 `ssh {host, port, user}`、结构化 `workspace` 与兼容字段 `workspace_path`；下面三组的每一行都只带 `server_id` 外键指回。
   - `gpus[]` 是**可申请卡**，只讲容量：`gpu_id`、`index`、`name`、`vram_mib` 和收敛后的中文 `status`（可申请一律为「可用」）。这里不返回遥测，也不返回 `keepalive`：未租出的卡上能观测到的负载来自 ServerPilot 自己的占卡程序（固定按总显存的 80% 持有），分配前会先停掉，把这份读数给调用者，只会让一张空闲卡看起来是满的。对这一组，调用者唯一要决策的是是否申请，容量信息足以支撑这个决策。
...
`no_capacity` 是结构化返回，不是工具错误：`gpu_apply` 拿不到卡时返回 `{"no_capacity": {reason, message, gpu_count, server_id}}`，`gpu_status` 在视野内一张可申请卡都没有时同样返回 `no_capacity`。两者都要当数据读，不要当成故障重试；只有 `Transport closed` 这类才是传输错误，最多重试一次。
