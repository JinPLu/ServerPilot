...
1. `gpu_status(server_id?, lease_id?)` 用三组分别回答三个不同的问题，遥测只出现在占用归属于调用者的那一组。**连接与工作目录逐服务器返回一次**：`servers[]` 给出 `server_id`、结构化 `ssh {host, port, user}`、结构化 `workspace` 与兼容字段 `workspace_path`；下面三组的每一行都只带 `server_id` 外键指回。
   - `gpus[]` 是**可申请卡**，只讲容量：`gpu_id`、`index`、`name`、`vram_mib` 和收敛后的中文 `status`（可申请一律为「可用」）。这里不返回遥测，也不返回 `keepalive`：未租出的卡上能观测到的负载是 ServerPilot 自己的占卡程序（固定按总显存的 80% 持有），分配前会先停止它，把这份读数投影给调用者只会让一张空闲卡看起来是满的。调用者在这一组上唯一能做的决策是申不申请，容量足以支撑它。
   - `busy_gpus[]` 是**别人占着的卡**：`gpu_id`、`index`、`status` 和人类可读 `task`，不含遥测——谁占着是可以据以决策的，别人的任务把卡用得多满则不是。因此判断“有没有空卡、被谁占着”仍然只需要这一次调用。
   ...
   它与 GUI 同读 daemon 的同一份 REST 快照，不会另行 SSH 采集；GUI 的逐卡瞬时遥测走各自的 REST 与进阶投影，不受本路径约束。首次只读采集还会将端点记录为 GPU、纯 CPU 或尚未确认；已识别纯 CPU 端点会在 `cpu_only_servers` 中返回服务器标识、在线状态、CPU 数和可用内存，但仅供说明，永不参与 GPU 分配。`workspace` 固定表达远端工作目录语义：`{path, kind=working_directory, use_as_cwd=true, code_location=not_provided}`。没有 GPU 时返回“无 GPU”；有 GPU 但一张都申请不到时返回 `no_capacity`。
