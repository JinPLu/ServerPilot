...
- 服务器分组与任务声明 GPU 需求已在源码落地，但尚未记录针对该合同的现场 daemon/MCP 联调；已安装连接器描述是否已刷新，不在本文件宣称范围内。
