...
本文只记录当前事实、直接证据和仍未验证的边界。源码与构建完成不等于本机正在运行或已部署。当前包装版本仍为 `1.9.1`；工作树已实现一等服务器分组与任务声明 GPU 需求，并去掉外部调度器提交、通用资源规划、工作负载预设和整套浏览器 `/ui`。本文件不宣称已重启 daemon、已重装 MCP 入口，或已重新生成现场连接器描述。下文 `1.7.0` 自动化、固定夹具桌面验收和 Windows 打包规格检查，以及 `1.6.0` 及更早的现场验收，仍是当时工作树的证据，与本次源码变更分开记录。历史过程见 `docs/archive/`。
